...
class CustomMASEnv(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = "rgb_array"):
        ...
        self.OUTPUT_NAME = 'test_torch_kakikae'

        # 目的地選択肢, 線分の始点と終点，(x1, y1, x2, y2)
        self.MAX_DESTINATION_NUM = 4 #目的地の選択肢の最大数
        ...
        # デフォルトの目的地リスト(queue)を作成 torch.catで結合 現在の目的地番号(1~目的地数)，目的地選択肢，終点
        self.DESTINATION_QUEUE1 = torch.cat([torch.tensor([1]), self.DESTINATION_OPT1, self.DESTINATION_OPT2, self.DESTINATION_OPT5, self.ENDPOINT_LEFT]) #左折
        self.DESTINATION_QUEUE2 = torch.cat([torch.tensor([1]), self.DESTINATION_OPT1, self.DESTINATION_OPT3, self.DESTINATION_OPT6, self.ENDPOINT_CENTER]) #直進
        self.DESTINATION_QUEUE3 = torch.cat([torch.tensor([1]), self.DESTINATION_OPT1, self.DESTINATION_OPT4, self.DESTINATION_OPT7, self.ENDPOINT_RIGHT]) #右折
        self.ALL_QUEUE = torch.stack([self.DESTINATION_QUEUE1, self.DESTINATION_QUEUE2, self.DESTINATION_QUEUE3])

        #dest_queuesに全ての情報を入れる
        self.destinations = torch.empty((self.PEOPLE_NUM, 4))
        self.destinations[:, :] = self.DESTINATION_QUEUE2[1:5]
        self.dest_queues = torch.empty((self.PEOPLE_NUM, 1 +  4 * self.MAX_DESTINATION_NUM))
        self.dest_queues[:, :] = self.DESTINATION_QUEUE2 #初期目的地は直進
        ...
        for i in range(self.PEOPLE_NUM):
            initial_state[i, 3] = torch.normal(torch.full((1, ), -1.3), 0.26)
    ...
    def step(self, action):
        ...
        if action == self.before_action: #誘導方向が変化しない場合何もしない
            ...
            self.dest_queues[self.dest_queues[:, 0] == 1, 1:] = self.DESTINATION_QUEUE1[1:]


        elif action == 1: #直進
            self.dest_queues[self.dest_queues[:, 0] == 1, 1:] = self.DESTINATION_QUEUE2[1:]

        elif action == 2: #左折
            self.dest_queues[self.dest_queues[:, 0] == 1, 1:] = self.DESTINATION_QUEUE3[1:]
        ...
        self.simu = simulator_mod_v2.Simulator(ped_ped=self.ped_ped, ped_space=self.ped_space, oversampling=2, delta_t=0.08, destinations=self.destinations, dest_queues=self.dest_queues, on_calc=self.on_calc)
        self.simu.integrator = simulator_mod_v2.PeriodicBoundary_mod(self.simu.integrator, x_boundary=[0.0, 18.0], y_boundary=[0.0, 24.0], destinations=self.destinations, dest_queues=self.dest_queues, action=action, ALL_QUEUE = self.ALL_QUEUE) # 周期的境界
