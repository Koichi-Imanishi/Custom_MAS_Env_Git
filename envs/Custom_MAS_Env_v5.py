...
class CustomMASEnv(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = "rgb_array"):
        ...
        self.RECORD_STEP_CYCLE = 3

        # 記録するエピソード周期
        self.RECORD_EPISODE_CYCLE = 1

        self.OUTPUT_NAME = 'test_waypoints'

        # 目的地選択肢, 線分の始点と終点，(x1, y1, x2, y2)
        self.DESTINATION_OPT1 = torch.tensor([6.0, 12.0, 12.0, 12.0])
        self.DESTINATION_OPT2 = torch.tensor([6.0, 6.0, 6.0, 12.0]) #左
        self.DESTINATION_OPT3 = torch.tensor([6.0, 6.0, 12.0, 6.0]) #中央
        self.DESTINATION_OPT4 = torch.tensor([12.0, 6.0, 12.0, 12.0]) #右
        self.DESTINATION_OPT5 = torch.tensor([0.0, 8.0, 0.0, 10.0]) #左出口
        self.DESTINATION_OPT6 = torch.tensor([8.0, 0.0, 10.0, 0.0]) #中央出口
        self.DESTINATION_OPT7 = torch.tensor([18.0, 8.0, 18.0, 10.0]) #右出口
        ...
        initial_state[:, 0:2] = (torch.rand((self.PEOPLE_NUM, 2))) * 5.8 + torch.tensor([6.1, 18.0])

        for i in range(self.PEOPLE_NUM):
            self.destinations[i, :] = self.dest_queues[i].popleft()
            initial_state[i, 3] = torch.normal(torch.full((1, ), -0.5), 0.1)
        ...
        self.upper_wall_1 = torch.stack([torch.linspace(6, 8, 20), torch.linspace(0, 0, 20)], -1) #固定壁　実際は細かい点の集合
        self.upper_wall_2 = torch.stack([torch.linspace(10, 12, 20), torch.linspace(0, 0, 20)], -1)
        self.upperleft_wall = torch.stack([torch.linspace(6, 6, 60), torch.linspace(0, 6, 60)], -1)
        self.upperright_wall = torch.stack([torch.linspace(12, 12, 60), torch.linspace(0, 6, 60)], -1)
        self.left_wall_1 = torch.stack([torch.linspace(0, 0, 20), torch.linspace(6, 8, 20)], -1)
        self.left_wall_2 = torch.stack([torch.linspace(0, 0, 20), torch.linspace(10, 12, 20)], -1)
        self.leftupper_wall = torch.stack([torch.linspace(0, 6, 60), torch.linspace(6, 6, 60)], -1)
        self.leftlower_wall = torch.stack([torch.linspace(0, 6, 60), torch.linspace(12, 12, 60)], -1)
        self.right_wall_1 = torch.stack([torch.linspace(18, 18, 20), torch.linspace(6, 8, 20)], -1)
        self.right_wall_2 = torch.stack([torch.linspace(18, 18, 20), torch.linspace(10, 12, 20)], -1)
        self.rightupper_wall = torch.stack([torch.linspace(12, 18, 60), torch.linspace(6, 6, 60)], -1)
        self.rightlower_wall = torch.stack([torch.linspace(12, 18, 60), torch.linspace(12, 12, 60)], -1)
        self.lowerleft_wall = torch.stack([torch.linspace(6, 6, 120), torch.linspace(12, 24, 120)], -1)
        self.lowerright_wall = torch.stack([torch.linspace(12, 12, 120), torch.linspace(12, 24, 120)], -1)
    ...
    def reset(
        ...
        *,
        ...
        self.info = {}
        self.info["left_people_num"] = 0
        self.info["center_people_num"] = 0
        self.info["right_people_num"] = 0
        self.info["action"] = []
        ...
        initial_state[:, 0:2] = (torch.rand((self.PEOPLE_NUM, 2))) * 6 + torch.tensor([6.0, 18.0])
    ...
    def step(self, action):
        ...
        terminated = self.is_terminated() #常にFalse
        #truncated = False
        truncated = self.is_truncated()

        self.info["action"].append(action)
        self.info["left_people_num"] = torch.sum((self.states[-1, 0] > 0.0) & (self.states[-1, 0] < 6.0) & (self.states[-1, 1] > 6.0) & (self.states[-1, 1] < 12.0)).item()
        self.info["center_people_num"] = torch.sum((self.states[-1, 0] > 6.0) & (self.states[-1, 0] < 12.0) & (self.states[-1, 1] > 0.0) & (self.states[-1, 1] < 6.0)).item()
        self.info["right_people_num"] = torch.sum((self.states[-1, 0] > 12.0) & (self.states[-1, 0] < 18.0) & (self.states[-1, 1] > 6.0) & (self.states[-1, 1] < 12.0)).item()

        reward = min(self.info["left_people_num"], self.info["center_people_num"], self.info["right_people_num"]) - max(self.info["left_people_num"], self.info["center_people_num"], self.info["right_people_num"])
