...
class CustomMASEnv(gym.Env):
    ...
    class Wall(pygame.sprite.Sprite):
        def __init__(self):
            pygame.sprite.Sprite.__init__(self, self.containers)
            self.image = pygame.Surface([200, 40], flags=pygame.SRCALPHA)
            self.image.fill((0, 0, 0, 0))
            pygame.draw.line(self.image, GREEN, (0, 20), (200, 20), 2)
        ...
        def update(self, action):
            ...
            if action == 0:
                ...
                pygame.draw.line(self.image, GREEN, (100, 0), (100, 40), 2)
            elif action == 2:
    ...
    def step(self, action):
        ...
        self.observation = self.states.to('cpu').detach().numpy().copy() #statesをnumpyに変換
        reward = np.linalg.norm(self.observation[:, :, 2:4], axis=-1).mean()
        print(reward)

        terminated = False #常にFalse
        truncated = True #常にTrue
        info = {}
